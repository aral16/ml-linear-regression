...
def preprocess(config_path: str = "config.yaml") -> PreprocessArtifacts:
    ...
    train_path = os.path.join(raw_dir, "ENB2012_data.xlsx")
    ...
    "Heating_Load",
    "Cooling_Load"
] # Here, we're renaming the column base on UCI metadata on their page for this data. We're doing this cause the xlsx format data didn't have the correct column name.
    ...
    X = df.drop(columns=[target_col,"Cooling_Load"])# Here we're  the target column, but also removing Cooling_Load column cause it,s correlated with our target. It could cause a leakage  features if i don't remove it.
