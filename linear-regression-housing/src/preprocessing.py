...
#     except Exception:
#         return ["feature_" + str(i) for i in range(preprocessor.transform(pd.DataFrame()).shape[1])]  # fallback


# def preprocess(config_path: str = "config.yaml") -> Tuple[PreprocessArtifacts, str]:
...
#             f"Expected Kaggle file at {train_path}. Put Kaggle train.csv into data/raw/."
#         )

#     df = pd.read_csv(train_path)
    

#     target_col = cfg.get("data", {}).get("target_col", "SalePrice")
#     if target_col not in df.columns:
#         raise ValueError(f"Target column '{target_col}' not found. Columns: {list(df.columns)[:20]}...")

#     id_col = cfg.get("data", {}).get("id", "Id")
#     if id_col not in df.columns:
#         raise ValueError(f"Id column '{id_col}' not found. Columns: {list(df.columns)[:20]}...")
...
#         ("scaler", StandardScaler()), # Here we're scaling , By Standardizing, (mean=0, std=1)
#     ])
...
#     # Fit on train only (avoid leakage), transform both
...
#     # Save arrays
#     np.save(os.path.join(processed_dir, "X_train.npy"), X_train.toarray() if hasattr(X_train, "toarray") else X_train)
#     np.save(os.path.join(processed_dir, "X_test.npy"), X_test.toarray() if hasattr(X_test, "toarray") else X_test)
#     np.save(os.path.join(processed_dir, "y_train.npy"), y_train)
#     np.save(os.path.join(processed_dir, "y_test.npy"), y_test)

#     # Save preprocessor (this replaces scaler.joblib)
...
#     )
#     return artifacts, preprocessor_path


# if __name__ == "__main__":
#     preprocess()
#     print("✅ Kaggle preprocessing complete. Saved processed arrays + preprocessor.")
